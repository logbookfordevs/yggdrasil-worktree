...
- **New official task:** use `yggtree wt create` when the work should get a new
  branch-backed worktree and a remote-tracked branch. Read
  `references/create-task-worktree.md`.
- **Existing branch or clean interruption:** use `yggtree wt worktree-checkout`
...
- **Local experiment:** use `yggtree wt create-sandbox` when the user wants to
...
- Prefer `wt create` for real new scopes that deserve their own branch and
  remote tracking.
...
yggtree wt exec my-feature codex --approval-mode auto
```

## Common Mistakes

- Do not use `wt create` for throwaway experimentation; it is the official
  branch-backed workflow and may publish to `origin`.
- Do not reach for `git stash` when the user really needs a second realm.
- Do not delete a sandbox before running `wt unapply` if the user may need to
